...
def vote_ensemble(filename, voter):
    ...
    for i in range(len(test)):
        ...
        for j in range(len(ticket)):
            if ticket[j][i] == 0:
                vote0 = vote0 + voter[j][2]
            if ticket[j][i] == 1:
                vote1 = vote1 + voter[j][2]
        if vote0 > vote1:
            answer.append(0)
        if vote1 > vote0:
            answer.append(1)
...
for i in range(1,101):
    ...
    predict = vote_ensemble("validationdata.txt", vote_nn)
    validation_label = read_label("validationlabel.txt")
    score = 0
    for i in range(len(predict)):
        if predict[i] == validation_label[i]:
            score = score + 1
    score = score / len(predict)
    print("-------------------------------------")
    print("Total voter validation: acc = %f, LR = %f" % (score, LR))
    print("-------------------------------------")
